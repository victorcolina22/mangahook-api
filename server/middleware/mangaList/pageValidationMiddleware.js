...
const pagesValidation = (req, res, next) => {
  ...
  const categoryId = req.metaData.category.filter((val) => {
    return val.name === req.query.category;
  })[0].id;
  httpReq(
    `${urlLink}/manga_list?type=${type}&category=${categoryId}&state=${state}&page=99999999`,
  )
    .then((html) => {
      const $ = cheerio.load(html);
      const totalStories = $(".panel_page_number .group_qty .page_blue")
        .text()
        .match(/Total:\s*([\d,]+)\s+stories/)[1];
      const totalPages = $(".panel_page_number .group_page .page_last")
        .text()
        .match(/Last\((\d+)\)/);
      ...
      req.metaData = {
        ...
        totalPages: totalPages
          ? Number(totalPages[1])
          : $(".panel_page_number .group_page .page_last").text(),
        ...req.metaData,
      };
